@php
    $isSiswaIstimewa = Auth::guard('siswa')->check()
        && Auth::guard('siswa')->user()->siswa?->nisn === '0078103635';
@endphp

{{-- Siswa istimewa: tampilkan galeri eksklusif tema hijau pondok --}}
@if($isSiswaIstimewa)
    @include('siswa.galeri_istimewa')
@else
@php
...
body::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;background:radial-gradient(ellipse 100% 50% at 50% -10%,rgba(201,168,76,.1) 0%,transparent 65%)}
...
.page-header h1{font-family:'Cormorant Garamond',serif;font-size:clamp(2rem,7vw,3.2rem);font-weight:700;line-height:1.1;margin-bottom:.5rem}
...
.go-nisn{font-size:.68rem;color:rgba(201,168,76,.85);margin-top:.15rem}
...
.lb-prev{left:.75rem}
.lb-next{right:.75rem}
.lb-prev:hover,.lb-next:hover{background:rgba(201,168,76,.35);border-color:var(--b1)}
...
.lb-dl-btn{
    ...
    background:rgba(201,168,76,.12);border:1px solid var(--b1);
    border-radius:.625rem;color:var(--gold2);
    font-size:.75rem;font-weight:600;cursor:pointer;text-decoration:none;
    transition:background .2s,transform .15s;white-space:nowrap;
}
.lb-dl-btn:hover{background:rgba(201,168,76,.22);transform:translateY(-1px)}
...
.pg-wrap .page-link:hover,.pg-wrap .page-item.active .page-link{background:rgba(201,168,76,.15);border-color:var(--gold)}
...
.bn-item.active .bn-icon-wrap{background:rgba(201,168,76,.12);border-radius:.625rem;padding:.35rem .75rem}
.bn-logout{color:#FCA5A5}
.bn-logout:hover{background:rgba(220,38,38,.08)}

/* ── Countdown ── */
.cd-banner{border-radius:1.25rem;padding:1.75rem 1.5rem;margin-bottom:2rem;background:linear-gradient(135deg,rgba(201,168,76,.08),rgba(201,168,76,.03));border:1px solid var(--b1);text-align:center;position:relative;overflow:hidden}
...
.cd-b{background:rgba(0,0,0,.35);border:1px solid var(--b1);border-radius:.875rem;padding:.75rem .25rem}
.cd-b-val{font-family:'Cormorant Garamond',serif;font-size:clamp(1.6rem,6vw,2.4rem);font-weight:700;color:var(--gold2);line-height:1;display:block}
...
<div class="main">
    <div class="page-header">
        <div style="display:flex;justify-content:center;margin-bottom:1.25rem">
            <div style="width:64px;height:64px;display:flex;align-items:center;justify-content:center">
                ...
                @else
                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="rgba(201,168,76,.5)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="3"/><path d="M3 9h18M9 21V9"/></svg>
                @endif
            </div>
        </div>
    ...
    @else
        <div class="empty">
            ...
            <a href="{{ route('siswa.dashboard') }}" style="display:inline-flex;align-items:center;gap:.5rem;margin-top:1.25rem;padding:.6rem 1.25rem;background:rgba(201,168,76,.08);border:1px solid rgba(201,168,76,.2);border-radius:.75rem;color:var(--gold2);font-size:.8rem;font-weight:600;text-decoration:none;transition:background .2s" onmouseover="this.style.background='rgba(201,168,76,.15)'" onmouseout="this.style.background='rgba(201,168,76,.08)'">
    ...
    <div class="lb-inner">
        ...
        <div class="lb-info">
            <div class="lb-avatar" id="lbAvatar">
                ...
                @else
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(201,168,76,.6)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>
                @endif
            </div>
            <div class="lb-meta">
